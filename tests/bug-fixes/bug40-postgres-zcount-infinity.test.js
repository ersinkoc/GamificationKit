...
describe('Bug Fix #40: Storage zcount with infinity values', () => {
  let storage;
  ...
  test('should count all elements with -inf to +inf', async () => {
    const count = await storage.zcount('test:leaderboard', '-inf', '+inf');
    expect(count).toBe(5);
  });

  test('should count elements from -inf to specific value', async () => {
    const count = await storage.zcount('test:leaderboard', '-inf', 30);
    expect(count).toBe(3);
  });

  test('should count elements from specific value to +inf', async () => {
    const count = await storage.zcount('test:leaderboard', 30, '+inf');
    expect(count).toBe(3);
  });

  test('should count elements within specific range', async () => {
    const count = await storage.zcount('test:leaderboard', 20, 40);
    expect(count).toBe(3);
  });

  test('should return 0 for empty range', async () => {
