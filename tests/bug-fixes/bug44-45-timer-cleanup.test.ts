...
describe('Bug Fix #44: StreakModule timer cleanup', (): void => {
  let streakModule;
  let storage;
  let eventManager;
  ...
  test('should store initialCheckTimeout reference', async (): Promise<void> => {
  ...
  test('should clear initialCheckTimeout on shutdown', async (): Promise<void> => {
    await streakModule.initialize();
  ...
  test('should not throw when shutdown called before timeout fires', async (): Promise<void> => {
    await streakModule.initialize();
...
describe('Bug Fix #45: QuestModule timer cleanup', (): void => {
  let questModule;
  let storage;
  let eventManager;
  ...
  test('should store initialRotationTimeout reference', async (): Promise<void> => {
  ...
  test('should clear initialRotationTimeout on shutdown', async (): Promise<void> => {
    await questModule.initialize();
  ...
  test('should not throw when shutdown called before rotation fires', async (): Promise<void> => {
    await questModule.initialize();
