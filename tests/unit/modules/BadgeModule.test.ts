...
describe('BadgeModule', (): void => {
  ...
  describe('award', (): void => {
    ...
    it('should award badge to user', async (): Promise<void> => {
    ...
    it('should prevent duplicate awards when maxAwards is 1', async (): Promise<void> => {
    ...
    it('should allow multiple awards when maxAwards > 1', async (): Promise<void> => {
      badgeModule.addBadge({
        id: 'multi-badge',
        name: 'Multi Badge',
        maxAwards: 3
    ...
    it('should emit badges.awarded event', async (): Promise<void> => {
    ...
    it('should handle badge not found', async (): Promise<void> => {
      await expect(
        badgeModule.award('user123', 'non-existent')
      ).rejects.toThrow('Badge not found: non-existent');
    });

    it('should not award disabled badges', async (): Promise<void> => {
      badgeModule.addBadge({
        id: 'disabled-badge',
        name: 'Disabled Badge',
        enabled: false
    ...
    it('should handle badges with expiration', async (): Promise<void> => {
      badgeModule.addBadge({
        id: 'expiring-badge',
        name: 'Expiring Badge',
  ...
  describe('revoke', (): void => {
    ...
    it('should revoke badge from user', async (): Promise<void> => {
    ...
    it('should emit badges.revoked event', async (): Promise<void> => {
      const emitSpy = jest.spyOn(eventManager, 'emitAsync');
    ...
    it('should handle badge not awarded', async (): Promise<void> => {
  ...
  describe('getUserBadges', (): void => {
    ...
    it('should return all user badges', async (): Promise<void> => {
    ...
    it('should include award metadata', async (): Promise<void> => {
      const badges = await badgeModule.getUserBadges('user123');
    ...
    it('should return empty array for user with no badges', async (): Promise<void> => {
      const badges = await badgeModule.getUserBadges('newuser');
      expect(badges).toEqual([]);
    });

    it('should filter out expired badges', async (): Promise<void> => {
  ...
  describe('hasBadge', (): void => {
    beforeEach(async () => {
      badgeModule.addBadge({ id: 'test-badge', name: 'Test Badge' });
      await badgeModule.award('user123', 'test-badge');
    });

    it('should return true if user has badge', async (): Promise<void> => {
      const hasBadge = await badgeModule.hasBadge('user123', 'test-badge');
      expect(hasBadge).toBe(true);
    });

    it('should return false if user does not have badge', async (): Promise<void> => {
      const hasBadge = await badgeModule.hasBadge('user456', 'test-badge');
      expect(hasBadge).toBe(false);
    });

    it('should return false for non-existent badge', async (): Promise<void> => {
      const hasBadge = await badgeModule.hasBadge('user123', 'non-existent');
      expect(hasBadge).toBe(false);
    });
  });

  describe('getAllBadges', (): void => {
    beforeEach(() => {
      badgeModule.addBadge({ 
        id: 'common1', 
        name: 'Common 1', 
    ...
    it('should return all non-secret badges by default', async (): Promise<void> => {
    ...
    it('should include secret badges when requested', async (): Promise<void> => {
  ...
  describe('progress tracking', (): void => {
    beforeEach(() => {
      badgeModule.addBadge({
        id: 'reader',
        name: 'Avid Reader',
    ...
    it('should track progress', async (): Promise<void> => {
    ...
    it('should auto-award when target reached', async (): Promise<void> => {
    ...
    it('should emit badges.progress.updated event', async (): Promise<void> => {
      const emitSpy = jest.spyOn(eventManager, 'emitAsync');
    ...
    it('should handle multiple progress requirements', async (): Promise<void> => {
      badgeModule.addBadge({
        id: 'multi-progress',
        name: 'Multi Progress',
        conditions: {
  ...
  describe('getUserStats', (): void => {
    ...
    it('should include progress for incomplete badges', async (): Promise<void> => {
      badgeModule.addBadge({
        id: 'progress-badge',
        name: 'Progress Badge',
        conditions: {
  ...
  describe('resetUser', (): void => {
    ...
    it('should emit badges.user.reset event', async (): Promise<void> => {
      const emitSpy = jest.spyOn(eventManager, 'emitAsync');
  ...
  describe('getBadgeStats', (): void => {
    ...
    it('should return badge statistics', async (): Promise<void> => {
