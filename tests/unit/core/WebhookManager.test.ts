...
describe('WebhookManager', (): void => {
  ...
  describe('trigger', (): void => {
    beforeEach(() => {
      jest.useFakeTimers();
    });

    it('should send webhook to matching URLs', async (): Promise<void> => {
    ...
    it('should support wildcard events', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: true,
        status: 200
      });
    ...
    it('should sign requests when secret is provided', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: true,
        status: 200
      });
    ...
    it('should skip inactive webhooks', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: true,
        status: 200
      });
    ...
    it('should retry failed requests', async (): Promise<void> => {
      global.fetch
    ...
    it('should use exponential backoff for retries', async (): Promise<void> => {
    ...
    it('should handle non-2xx status codes', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: false,
        status: 404,
  ...
  describe('activate/deactivate', (): void => {
  ...
  describe('error handling', (): void => {
    it('should handle malformed URLs gracefully', async (): Promise<void> => {
    ...
    it('should handle circular reference in payload', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: true,
        status: 200
      });
  ...
  describe('batch operations', (): void => {
    it('should handle multiple webhooks for same event', async (): Promise<void> => {
      global.fetch.mockResolvedValue({
        ok: true,
        status: 200
      });
    ...
    it('should continue sending if one webhook fails', async (): Promise<void> => {
      global.fetch
